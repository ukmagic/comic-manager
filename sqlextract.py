...
MODELS = {
    ...
    "gcd_group_membership_type": GroupMembershipType.fromGCD,
    "gcd_series": Series.fromGCD,
    "gcd_issue": Comic.fromGCD,
    "gcd_story_type": StoryType.fromGCD,
    "gcd_character_role": Role.fromGCD,
    "gcd_story": Story.fromGCD,
    "gcd_story_credit": updateStoryCredits,
    "gcd_feature_type": FeatureType.fromGCD,
    "gcd_feature": Feature.fromGCD,
    "gcd_multiverse": Multiverse.fromGCD,
    "gcd_universe": Universe.fromGCD,
    "gcd_character": CharacterDetails.fromGCD,
    "gcd_character_name_detail": CharacterName.fromGCD,
    "gcd_group": Group.fromGCD,
    "gcd_group_character": GroupCharacter.fromGCD,
    "gcd_story_character": StoryCharacter.fromGCD,
    "gcd_character_relation_type": RelationType.fromGCD,
    "gcd_group_relation_type": RelationType.fromGCD,
    "gcd_character_relation": CharacterRelation.fromGCD,
    "gcd_group_relation": GroupRelation.fromGCD,
    "gcd_group_membership": GroupMembership.fromGCD,
    "gcd_story_character_group": StoryCharacterGroup.fromGCD,
}
...
def gcd_story(values):
    #if values[0] in {1478392, 2782857, 2782856, 2782859, 2782858, 2782850}:
    #    return values
...
def gcd_character_relation(values):
    return values


def gcd_character_relation_type(values):
    return values


def gcd_group_relation(values):
    return values
...
def gcd_character_name_detail(values):
    return values


def gcd_character(values):
    if not exists("stddata_language", values[9]):
        return
    return values


def gcd_group(values):
    if not exists("stddata_language", values[9]):
        return
    return values


def gcd_universe(values):
    return values


def gcd_multiverse(values):
    return values


def gcd_group_name_detail(values):
    return values


def gcd_story_character(values):
    return values


def gcd_group_character(values):
    return values


def gcd_character_role(values):
    return values


def gcd_story_character_group(values):
    return values


def gcd_group_membership_type(values):
    return values


def gcd_group_membership(values):
    return values
...
CREATE = {
    ...
    "gcd_character_relation": gcd_character_relation,
    "gcd_issue_credit": gcd_issue_credit,
    "gcd_printer": gcd_printer,
    "gcd_character_name_detail": gcd_character_name_detail,
    "gcd_character": gcd_character,
    "gcd_group": gcd_group,
    "gcd_universe": gcd_universe,
    "gcd_multiverse": gcd_multiverse,
    "gcd_group_name_detail": gcd_group_name_detail,
    "gcd_story_character": gcd_story_character,
    "gcd_group_character": gcd_group_character,
    "gcd_group_relation" : gcd_group_relation,
    "gcd_character_relation": gcd_character_relation,
    "gcd_character_relation_type": gcd_character_relation_type,
    "gcd_group_relation_type": gcd_character_relation_type,
    "gcd_group_relation": gcd_group_relation,
    "gcd_character_role": gcd_character_role,
    "gcd_story_character_group": gcd_story_character_group,
    "gcd_group_membership_type": gcd_group_membership_type,
    "gcd_group_membership": gcd_group_membership,
...
def gen():
    ...
    for table in MODELS:
        ...
        print(table, len(tables[table]["data"]))
        ...
        for n, line in enumerate(f):
            ...
            if line.startswith("INSERT INTO") and table:
                ...
                for i,values in enumerate(data):
                    #if not output: # and table not in {"gcd_creator", "gcd_story", "gcd_creator_name_detail"}:
                    #    continue
                    if values is None:
                        continue
                    try:
